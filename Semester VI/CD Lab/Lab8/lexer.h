...
void insert(char *lex, char *type, int size, char *rettype, char *params) {
    ...
    newnode->sym = s; newnode->next = symTable[val].head;
    symTable[val].head = newnode; symTable[val].count++;
}
...
Token getNextToken(FILE *fp, int *row, int *col, char* dbuf) {
    ...
    while (1) {
        ...
        if (strchr(STRING_DELIMITERS, c)) 
            return handleString(fp, row, col, c);
